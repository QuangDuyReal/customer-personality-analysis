---
title: "PHÂN TÍCH DỮ LIỆU TÍNH CÁCH KHÁCH HÀNG CÁC PHÂN KHÚC ĐỂ ĐƯA RA CHIẾN LƯỢC KINH DOANH"
author: "Nhóm 01: 
...
"
github: "https://github.com/QuangDuyReal/customer-personality-analysis"
output:
  word_document:
    toc: true
    toc_depth: '3'
...
Bộ dữ liệu gốc bao gồm `ncol(customers_raw)` biến và `nrow(customers_raw)` quan sát. Các biến số cung cấp thông tin đa dạng về khách hàng, có thể được nhóm thành các loại chính như sau:
...
| (Lưu ý: `Recency`, `Education`, `Marital_Status`, `Age`, `NumWebVisitsMonth` không có ý nghĩa thống kê rõ rệt trong mô hình này) |  |  |
...
-   **Các yếu tố làm giảm chi tiêu đáng kể:** Có nhiều con (`Child_Total`), việc khách hàng phàn nàn (`Complain`), và thuộc Phân khúc K-Means 4 (so với Cụm 1).

-   Các yếu tố như `Age`, `Recency`, các mức độ `Education` và `Marital_Status` (so với mức tham chiếu "`Absurd`" và "`2n_Cycle`") không cho thấy ảnh hưởng có ý nghĩa thống kê rõ rệt đến `log_total_spent` trong mô hình này sau khi đã kiểm soát các yếu tố khác.
...
-   **Nguyễn Văn Quang Duy (23110086):**
    -   Phụ trách nghiên cứu, triển khai mã nguồn R, phân tích lý thuyết, kết quả và diễn giải các hệ số cho mô hình Hồi quy Logistic (dự đoán biến `Response`).
    -   Đóng góp vào việc đánh giá hiệu suất của mô hình này.
-   **Phan Trọng Phú (23133056):**
    -   Phụ trách nghiên cứu, triển khai mã nguồn R, phân tích lý thuyết, kết quả và diễn giải các hệ số cho mô hình Hồi quy Tuyến tính Đa biến (dự đoán biến `log_total_spent`).
    -   Đóng góp vào việc kiểm tra các giả định của mô hình này.
-   **Phan Trọng Quí (23133061):**
    -   Phụ trách nghiên cứu, triển khai mã nguồn R và phân tích lý thuyết, kết quả cho mô hình Phân cụm K-Means.
...
1.  **Đỗ Kiến Hưng:**
    -   **Nội dung triển khai được:** Tổng hợp và viết báo cáo chính, bao gồm các phần lý thuyết chung, mô tả dữ liệu, EDA, kết luận, và các phần phụ.
    -   **Mức độ hoàn thành:** Tốt
    -   **Ưu điểm:** Khả năng tổng hợp thông tin tốt, đảm bảo tiến độ chung của báo cáo.
2.  **Nguyễn Văn Quang Duy:**
    -   **Nội dung triển khai được:** Xây dựng và phân tích mô hình Hồi quy Logistic. Quản lý mã nguồn GitHub.
    -   **Mức độ hoàn thành:** Tốt
