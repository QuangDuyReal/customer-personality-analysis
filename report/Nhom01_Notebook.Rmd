...
-   **Các yếu tố làm giảm chi tiêu đáng kể:** Có nhiều con (`Child_Total`), việc khách hàng phàn nàn (`Complain`), và thuộc Phân khúc K-Means 4 (so với Cụm 1). Các yếu tố như `Age`, `Recency`, các mức độ `Education` và `Marital_Status` (so với mức tham chiếu "`Absurd`" và "`2n_Cycle`") không cho thấy ảnh hưởng có ý nghĩa thống kê rõ rệt đến `log_total_spent` trong mô hình này sau khi đã kiểm soát các yếu tố khác.
